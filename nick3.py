...
class Player(Character):
  def __init__(self):
    ...
    self.health_max = 10
  def quit(self):
    print "%s gets out of building but is attacked by millions of zombies and dies.\nR.I.P." % self.name
    self.health = 0
  def help(self): print Commands.keys()
  def status(self): print "%s's health: %d/%d" % (self.name, self.health, self.health_max)
  def tired(self):
    print "%s has suddenly become tired and needs to rest." % self.name
  ...
  def rest(self):
    if self.state != 'normal': print "%s can't rest now!" % self.name; self.enemy_attacks()
      ...
      if randint(0, 1):
        ...
        else: print "%s has overslept." % self.name; self.health = self.health - 1
  def explore(self):
    if self.state != 'normal':
      print "%s can't do that right now!" % self.name
      self.enemy_attacks()
    else:
      print "%s opens door to a new room." % self.name
      if randint(0, 1):
        self.enemy = Enemy(self)
  ...
  def attack(self):
    if self.state != 'fight': print "%s swats the air, without notable results." % self.name; self.tired()
      ...
      if self.do_damage(self.enemy):
        print "%s has killed a %s!" % (self.name, self.enemy.name)
        self.enemy = None
        self.state = 'normal'
...
p = Player()
p.name = raw_input("What do you want the game to call you? ")
#print "(type help to get a list of actions)\n"
#print "%s enters a tall building, trying to escape zombies." % p.name
 
while(p.health > 0):
  print("type help for a list of usable commands")
  line = raw_input("> ")
  args = line.split()
  if len(args) > 0:
    commandFound = False
    for c in Commands.keys():
      if args[0] == c[:len(args[0])]:
        ...
        break
    if not commandFound:
      print "%s type help to get a list of usable commands" % p.name
